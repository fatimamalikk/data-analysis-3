...
price_vs_property_box <- ggplot(data = data, aes(x = property_type, y = price)) +
  ...
  scale_y_continuous(expand = c(0.01,0.01),limits = c(0,300), breaks = seq(0,550,50)) +
...
loru <- data %>% filter(f_neighbourhood_group_cleansed=="Brindisi") %>% group_by(property_type, n_accommodates) %>% 
  summarise(count=n())
####
...
b <- data_holdout_w_prediction %>%
  filter(f_neighbourhood_group_cleansed %in% c("Foggia", "Lecce",
